<template>
    <div>
      <span class='icons' @click="selecter">
        <img :src="img" alt="">
      </span>
      <span class='chengshi' @click="selecter">{{city}}<i class="iconfont">&#xe6d1;</i></span>
      <transition name="fade">
        <div class="select-city-box" v-show="selectList">
          <HeaderBar>
            <a href="javascript:;" class="close iconfont" @click="selecter" slot="right-group">&#xe72a;
            </a>
          </HeaderBar>
          <div class="select-city-group">
            <div> 
              <div class="select-city-list" v-for="(item, key) in selectData" :key="key">
                <h3 class="title">{{key}}</h3>
                <ul class="list">
                  <li class="item" v-for="(val,index) in item" :key="index" @click="selectC(val)">{{val.split('|')[0]}}</li>
                </ul>         
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    Allcity () {
      this.selectData['热门'] = []
      for (let i = 0; i < 6; i++) {
        this.selectData['热门'].push(allCities[i])
      }
      for (let i = 0; i < this.word.length; i++) {
        let word = this.word.charAt(i)
        this.selectData[word] = []
        for (let k = 0; k < allCities.length; k++) {
          let firstword = allCities[k].split('|')[1].charAt(0).toLocaleUpperCase()
          if (firstword === word) {
            this.selectData[word].push(allCities[k])
          }
        }
      }
    },
  ...
  mounted () {
    this.selectListBox = this.$el.querySelector('.select-city-box')
